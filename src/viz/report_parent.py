...
def _generate_coach_note(metrics: PitcherMetrics, grades: dict[str, str], pitcher_name: str = "your pitcher") -> str:
...
def _coach_note_api(metrics: PitcherMetrics, grades: dict[str, str], pitcher_name: str, api_key: str) -> str:
    import anthropic
...
def _coach_note_offline(grades: dict[str, str], pitcher_name: str = "your pitcher") -> str:
    ...
    parts: list[str] = []
    if green_items:
        parts.append(f"Great work — {pitcher_name} is showing solid mechanics in {', '.join(green_items[:2])}.")
    else:
        parts.append(f"{pitcher_name.title()} is building a good foundation — keep focusing on movement quality over velocity.")
    if focus_items:
        parts.append(f"The main area to focus on this week is {focus_items[0]}. Use the drill below to work on it during warm-ups.")
    else:
...
def _build_injury_watch(metrics: PitcherMetrics) -> str:
    ...
    if ef is not None and (ef < 60 or ef > 120):
        warnings.append("Elbow angle at foot plant is outside the safe range (60–120°). This can increase stress on the elbow ligament.")
    sa = metrics.shoulder_abduction_fp
    if sa is not None and sa > 120:
        warnings.append("Arm is too high at foot plant (above 120°). This 'high-elbow' pattern can stress the shoulder.")
    stride = metrics.stride_length_pct_height
    if stride is not None and stride < 60:
        warnings.append("Stride is very short — the arm may have to work harder to compensate. Focus on getting more out of the legs.")
    if not warnings:
        return ""
...
""")
    ...
    parts.append(f'<div class="subtitle">{escape(video_filename)}{date_str} · {hand_label}</div>\n')
    css_grades: dict[str, str] = {}
    for metric_name in GRADE_RULES:
        css_grades[metric_name] = _grade_class(metric_name, metrics)
    pz_grades: dict[str, str] = {m: _grade_to_pitchzone(g) for m, g in css_grades.items()}
    pitchzone_svg = generate_pitchzone_svg(grades=pz_grades, metrics={m: getattr(metrics, m, None) for m in GRADE_RULES}, throws=throws, title="PitchZone")
    ...
    if foot_plant_overlay_b64:
        ...
        parts.append(f'<img src="data:image/png;base64,{foot_plant_overlay_b64}" alt="Foot Plant with Overlay">\n')
